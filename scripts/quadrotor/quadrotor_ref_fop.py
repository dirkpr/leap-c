"""Script to run sac_fop for the quadrotor tasks."""
import datetime
...
if __name__ == "__main__":
    ...
    cfg.sac.lr_pi = 1e-4  # 3e-5
    cfg.sac.lr_q = 1e-4  # 3e-5
    cfg.sac.lr_alpha = 1e-3
    cfg.sac.init_alpha = 0.0001
    cfg.sac.tau = 0.005
    cfg.sac.num_critics = 2
    cfg.sac.num_threads_mpc = 4

    output_path = Path(f"output/quadrotor_ref_hard/fop_{args.seed}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}")
    main("sac_fop", "quadrotor_ref_hard", cfg, output_path, args.device)
