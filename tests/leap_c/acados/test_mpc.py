...
def test_statelessness_batched(
    ...
    # Create MPC with some stateless and some global parameters
    lin_mpc = learnable_point_mass_mpc_different_params
    n_batch = lin_mpc.N_max_batch
    x0 = np.tile(x0, (lin_mpc.N_max_batch, 1))
    u0 = np.tile(u0, (lin_mpc.N_max_batch, 1))
    mpc_input_standard = AcadosOcpInput(x0=x0, u0=u0)
...
def test_closed_loop(
    ...
    for step in range(100):
        # first bring it in batch form
        batch_x = x[-1].reshape(1, -1)
        batch_p_global = p_global.reshape(1, -1)

        u_star, _, status = learnable_linear_mpc.policy(batch_x, p_global=batch_p_global)
