...
import numpy as np
import torch
from acados_template import AcadosOcp, AcadosOcpBatchSolver, AcadosOcpSolver, AcadosOcpFlattenedIterate, \
    AcadosMultiphaseOcp
...
AcadosOcpSolverState = AcadosOcpFlattenedBatchIterate
...
class AcadosOcpParameter(NamedTuple):
    """
    A named tuple to store the parameters of the MPC planner.
    NOTE: If the non-sensitivity acados is using something else than EXTERNAL cost, like LLS cost, then
    ...
    in that acados). This is due to the fact that "unused" (wrt. casadi expressions) p_global entries are currently not allowed in the acados.
    ...
    def get_sample(self, i: int) -> "AcadosOcpParameter":
    ...
    def ensure_float64(self) -> "AcadosOcpParameter":
        def convert(k, v):
        ...
        return AcadosOcpParameter(**kw)


class AcadosOcpInput(NamedTuple):
    """
    ...
    parameters: AcadosOcpParameter | None = None

    def is_batched(self) -> bool:
        return self.x0.ndim == 2

    def get_sample(self, i: int) -> "AcadosOcpInput":
        """Get the sample at index i from the batch."""
        if not self.is_batched():
        ...
        return AcadosOcpInput(x0=x0, u0=u0, parameters=parameters)


class AcadosOcpOutput(NamedTuple):
    """
    A named tuple to store the solution of the MPC planner.

    Attributes:
        status: The status of the acados.
...
def _solve_shared(
    ...
    # Else no iterate is used, which means the iterate of the resetted acados is used (i.e. all iterates are set to 0).
    ...
    if sensitivity_solver is not None:
        ...
        if mpc_input.parameters is not None:
            sens_input = AcadosOcpInput(
                x0=mpc_input.x0,
                u0=mpc_input.u0,
                parameters=AcadosOcpParameter(
...
) -> Callable[[AcadosOcpInput], AcadosOcpSolverState]:
    """Create a function that initializes the acados iterate with zeros.

    Args:
        solver: The acados to initialize.

    Returns:
        The function that initializes the acados iterate with zeros.
    """
...
class AcadosOcpSolverManager(ABC):
    ...
    def __init__(
        ...
        if ocp_sensitivity is None:
            # setup OCP for sensitivity acados
    ...
    @cached_property
    def default_full_mpcparameter(self) -> AcadosOcpParameter:
    ...
    @cached_property
    def default_sens_mpcparameter(self) -> AcadosOcpParameter:
        """Return the default MpcParameter for sensitivity acados.
    ...
    def __call__(
        ...
        Args:
            ...
            mpc_state: The iterate of the acados to use as initialization. If None, the
                acados is initialized using its init_state_fn.
    ...
    def _solve(
        ...
        return AcadosOcpOutput(**kw), flat_iterate

     # TODO (Jasper): Move this somewhere else this is not a class method.
    def last_solve_diagnostics(
        self, ocp_solver: AcadosOcpBatchSolver
    ) -> dict | list[dict]:
        ...
        if isinstance(ocp_solver, AcadosOcpBatchSolver):
            ...
            raise ValueError(
                f"Unknown acados type, expected AcadosOcpSolver or AcadosOcpBatchSolver, but got {type(ocp_solver)}."
            )


def set_ocp_solver_iterate(
    ocp_solver: AcadosOcpSolver | AcadosOcpBatchSolver,
    ocp_iterate: AcadosOcpSolverState | None,
) -> None:
...
def set_ocp_solver_mpc_params(
    ocp_solver: AcadosOcpSolver | AcadosOcpBatchSolver,
    mpc_parameter: AcadosOcpParameter | None,
) -> None:
...
def initialize_ocp_solver(
    ...
    """Initializes the fields of the OCP (batch) acados with the given values.

    Args:
        ocp_solver: The OCP (batch) acados to initialize.
        mpc_input: The MPCInput containing the parameters to set in the OCP (batch) acados
            and the state and possibly control that will be used for the initial conditions.
        ocp_iterate: The iterate of the acados to use as initialization.
        set_params: Whether to set the MPC parameters of the OCP (batch) acados.
        throw_error_if_u0_is_outside_ocp_bounds: If True, an error will be thrown when given an u0 in mpc_input that
...
def unset_ocp_solver_initial_control_constraints(
    ...
    """Unset the initial control constraints of the OCP (batch) acados."""
...
def set_ocp_solver_to_default(
    ocp_solver: AcadosOcpSolver | AcadosOcpBatchSolver,
    default_mpc_parameters: AcadosOcpParameter,
    unset_u0: bool,
) -> None:
    """Resets the OCP (batch) acados to remove any "state" to be carried over in the next call.
    Since the init function or a given iterate is being used to override the state of the acados anyways,
